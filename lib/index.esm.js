...
function getUrlParam(sUrl, sKey) {
  var result = {};
  sUrl.replace(/\??(\w+)=([^&]*)&?/g, function (a, k, v) {
...
function getPerformance(_a) {
  ...
  var firstPaintTime;
  var firstContentfulPaintTime; // 是否已经形成数据（页面加载完成之后）
...
function addInlineStyle(_a) {
  ...
  var customStyle = document.createElement('style'); // 如果需要 ID

  if (id) {
    domId = "" + id;
    idDom = document.getElementById(domId); // 如果 Dom 不存在，插入 style
...
function customScrollBarForTransformEle(_a) {
  var _b = _a === void 0 ? {} : _a,
      _c = _b.containerClassName,
      containerClassName = _c === void 0 ? '' : _c,
      _d = _b.imgBoxClassName,
      imgBoxClassName = _d === void 0 ? '' : _d,
      _e = _b.imgBoxDom,
      imgBoxDom = _e === void 0 ? null : _e,
      _f = _b.imgClassName,
      imgClassName = _f === void 0 ? '' : _f,
      _g = _b.imgDom,
      imgDom = _g === void 0 ? null : _g,
      _h = _b.action,
      action = _h === void 0 ? '' : _h,
      _j = _b.customStyle,
      customStyle = _j === void 0 ? '' : _j; // 变量


  var barIsHideId = 'bar-is-hide';
  var barTopId = 'bar-top';
  var barBottomId = 'bar-bottom';
  ...
  setTimeout(function () {
    ...
    var imgInstance = imgDom || document.querySelector("." + imgClassName);
    ...
    var imgBoxInstance = imgBoxDom || document.querySelector("." + imgBoxClassName);
    ...
    var Box2ToRes = getBox2To(); // console.log('获取图片距离顶部的距离', Box2ToRes);

    var ImgHeightRes = getImgHeight(); // console.log('获取图片的高度', ImgHeightRes);

    var WindowHeightRes = getWindowHeight(); // console.log('获取窗口的高度', WindowHeightRes);

    var TopOverflowPercentRes = getTopOverflowPercent(); // console.log('获取顶部溢出的百分比', TopOverflowPercentRes);

    var BottomOverflowPercentRes = getBottomOverflowPercent(); // console.log('获取顶部溢出的百分比', BottomOverflowPercentRes);

    var hideBarRes = hideBar(); // console.log('如果都是 0 ，隐藏滚动条', hideBarRes);

    if (!hideBarRes) {
      setBarTop(); // const setBarTopRes = setBarTop();
      // console.log('设置顶部状态栏', setBarTopRes);

      setBarBottom(); // const setBarBottomRes = setBarBottom();
      // console.log('设置底部状态栏', setBarBottomRes);
    } // 获取图片距离顶部的距离


    function getBox2To() {
      var translates = window.getComputedStyle(imgBoxInstance, null).transform;
      var tanslateY = parseFloat(translates.substring(6).split(',')[5]);
      return tanslateY;
    } // 获取图片的高度
    ...
    function getImgHeight() {
      ...
      return (_a = imgInstance) === null || _a === void 0 ? void 0 : _a.clientHeight;
    } // 获取窗口的高度


    function getWindowHeight() {
      return document.querySelector("." + containerClassName).clientHeight; // window.outerHeight;
    } // 获取顶部溢出的百分比


    function getTopOverflowPercent() {
      var box2To = Box2ToRes;

      if (box2To >= 0) {
        box2To = 0;
      }

      box2To = Math.abs(box2To); // 图片高度

      var imgHeight = ImgHeightRes;
      var per = floatToPercent(box2To / imgHeight, 2);
      return {
        box2To: box2To,
        per: per
      };
    ...
    function getBottomOverflowPercent() {
      // 窗口高度
      var windowHeight = WindowHeightRes; // 图片高度

      var imgHeight = ImgHeightRes; // 上面溢出的高度

      var box2To = Box2ToRes; // 底部

      var remindImageHeight = windowHeight - box2To;
      var box2ToBottom = imgHeight - remindImageHeight;

      if (box2ToBottom <= 0) {
        box2ToBottom = 0;
      }

      var per = floatToPercent(box2ToBottom / imgHeight, 2);
      return {
        box2ToBottom: box2ToBottom,
        per: per
      };
    } // 如果都是 0 ，隐藏滚动条
    ...
    function hideBar() {
      var box2To = TopOverflowPercentRes.box2To;
      var box2ToBottom = BottomOverflowPercentRes.box2ToBottom;
    ...
    function setBarTop() {
      var per = TopOverflowPercentRes.per;
      return addInlineStyle({
        inlineStyle: "\n          ." + containerClassName + "::after {\n            top: " + per + ";\n          }\n        ",
        id: barTopId
      });
    ...
    function setBarBottom() {
      var per = BottomOverflowPercentRes.per;
      return addInlineStyle({
        inlineStyle: "\n          ." + containerClassName + "::after {\n            bottom: " + per + ";\n          }\n        ",
        id: barBottomId
      });
...
function calcContainImageSizeAndPosition(_a) {
  var _b = _a === void 0 ? {} : _a,
      _c = _b.oriImageWidth,
      oriImageWidth = _c === void 0 ? 0 : _c,
      _d = _b.oriImageHeight,
      oriImageHeight = _d === void 0 ? 0 : _d,
      _e = _b.viewportWidth,
      viewportWidth = _e === void 0 ? 0 : _e,
      _f = _b.viewportHeight,
      viewportHeight = _f === void 0 ? 0 : _f;

  var targetImageWidth = viewportWidth;
  var _g = [0, 0, 0],
      targetImageHeight = _g[0],
      top = _g[1],
      wPer = _g[2]; // 比例
  ...
  if (targetImageHeight < viewportHeight) {
    var doubleGap = viewportHeight - targetImageHeight;
    top = doubleGap / 2;
  }
...
export { addClass, addInlineStyle, calLongestCommonSubsequence, calLongestCommonSubstring, calcContainImageSizeAndPosition, camelCase2Underscore, camelCaseToKebabCase, cancelBubble, clearHtml, compatibleExist, customScrollBarForTransformEle, cutCHSString, debounce, deepCopyObject, doFn, floatFixed, floatToPercent, friendlyInterval, genCustomConsole, generateRndNum, generateUniqueNum, getBrowserType, getCookie, getDomain, getHashQueryParam, getLocalStorage, getPerformance, getQueryParam, getSearchQueryParam, getSessionStorage, getUrlParam, hasClass, inRate, isJsonString, isNumber, isSafePWAEnv, join, loadCSS, loadScript, mNow, newLine, removeClass, renderTable, resetForm, setCookie, setLocalStorage, setSessionStorage, throttle, trim$1 as trim, updateQueryStringParameter, windowLoaded };
